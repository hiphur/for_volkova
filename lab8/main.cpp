...
public:
    void generate(const char* filename) {
        ...
        if (!out.is_open()) throw runtime_error("err wg");
        ...
        for (int i = 0; i < n + 2; ++i) {
            temp[i] = new char[m + 2];
            for (int j = 0; j < m + 2; ++j) {
                if (i == 0 || i == n + 1 || j == 0 || j == m + 1) temp[i][j] = '*';
                else temp[i][j] = ' ';
            }
        }
        ...
        for (int i = 0; i < n + 2; ++i) {
            for (int j = 0; j < m + 2; ++j) out << temp[i][j];
            out << "\n";
            delete[] temp[i];
        }
        delete[] temp;
...
public:
    ...
    void push(class BaseMonster* m) {
        try {
            ...
            throw runtime_error("err q_push");
        }
    }
...
public:
    Arena(const char* filename) {
        ...
        try {
            ...
            throw runtime_error("err a_init");
        }
    }
    ...
    void playRound(string* logs, int& log_count) {
        try {
            ...
            throw runtime_error("err play_rnd");
        }
    }
...
void BaseMonster::baseMoveLogic(Arena& arena, string* logs, int& log_count) {
    ...
    if (isPlayerControlled()) {
        ...
        cout << symbol << "? ";
        int dir;
        if (!(cin >> dir)) throw runtime_error("err in");
        if (dir == 1 || dir == -1) {
            direction = dir;
        } else {
