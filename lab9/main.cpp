...
struct Queue{
    int mons = 6;
    int head, tail, cnt = 0;
    BaseMonster **q = new BaseMonster*[mons];

    void fill(space* N); // заполнить очередь монстрами
    void addMonster(space* N);
    void push(BaseMonster* m); // добавить в конец очереди

    BaseMonster* pop(); // получить и убрать из головы очереди
...
public:
    ...
    ~space();
    void out(){ lock_guard<recursive_mutex> lock(mFL); cout << *this; };
    ...
    // 9
    void contrAttack(BaseMonster* mon1, BaseMonster* mon2);
    int monDead(BaseMonster* mon1, pos &Pmon1, BaseMonster* mon2, pos &Pmon2);
    void monsterThread(BaseMonster* mon);
    recursive_mutex mFL;
...
class MonsterA : public BaseMonster{ public: MonsterA() : BaseMonster(15,5,3,'A'){} }; // hp dmg step
...
void Queue::delMon(BaseMonster *arr[], BaseMonster* mon, int& monCount){
    ...
    if (findMon == -1) return; // не нашли такой позиции монстра в очереди

    // сдвигаем элементы от найденной позиции до tail-1 влево на одну позицию
    for (int i = findMon; i < cnt - 1; i++){
        int cur = (head + i) % mons;
        int nxt = (head + i + 1) % mons;
        q[cur] = q[nxt];
    }
    // корректируем tail и размер
...
void space::DirectionLowReg(int step, pos &PUniMon, BaseMonster* mon, int indI) {
    ...
    for (int i = indI; i < step; i++) {
        lock_guard<recursive_mutex> lock(mFL);
        bool aF = false;
        if (Data[PUniMon.y][PUniMon.x + t] == ' ') {
            ...
            afterFight: // если монстр выиграет бой, продолжить движение вверх/вниз
    ...
    if (dirX == 1 || dirX == -1){
        for (int i = indI; i < step; i++){
            if (Data[PUniMon.y][PUniMon.x + dirX] == ' ') {
                ...
                afterBigFight: // если монстр выиграет бой, продолжить движение вверх/вниз
...
void space::attack(BaseMonster* mon1, pos &Pmon1, BaseMonster* mon2, pos &Pmon2){
    while (mon1->getHealth() > 0 && mon2->getHealth() > 0){
        ...
        contrAttack(mon2, mon1); // mon2 контр-атакует mon1
        ...
        contrAttack(mon1, mon2); // mon1 контр-атакует mon2

        if (monDead(mon1, Pmon1, mon2, Pmon2)) break;
    }
}

void space::contrAttack(BaseMonster* mon1, BaseMonster* mon2){
    mon2->takeDamage(mon1->getDmg());
    L.outLogs(string(1, mon1->getName()) + " Контр-атакует " + string(1, mon2->getName()) + " на " + to_string(mon1->getDmg()) + " урона. " + "Здоровье " + string(1, mon2->getName()) + ": " + to_string(mon2->getHealth()));
}
...
int space::findSamePos(const pos &p){ // поиск монстра по позиции, возвращает индекс
...
void space::monsterThread(BaseMonster* mon){
    while (!gameFinished.load() && isAlive(mon)) {
        this_thread::sleep_for(chrono::milliseconds(1000));


        if (gameFinished.load() || !isAlive(mon)) break;

        {
            lock_guard<recursive_mutex> lock(mFL);

            // cout << "Поток " << this_thread::get_id() << "и монстр " << mon->getName() << endl;

            move(mon->location, mon);
...
void space::beginGame(){
    ...
    int PCMonCount = 0;
    for (int i = 0; i < monCounter; i++){
        if (!isupper(monsters[i]->getName()) && isAlive(monsters[i])){
            PCMonCount++;
        }
    }

    thread* threads = new thread[PCMonCount];
    int thInd = 0;

    for (int i = 0; i < monCounter; i++) {
        BaseMonster* m = monsters[i];
        if (!isupper(m->getName())) {
            threads[thInd++] = thread(&space::monsterThread, this, m);
        }
    }
    ...
    while (true) {
        {
            ...
            if (countPlayer == 0) { cout << "Монстры робота победили!" << endl; gameFinished = true; break; }
            if (countRobot == 0) { cout << "Монстры игрока победили!" << endl; gameFinished = true; break; }
        }
        ...
        for (int i = 0; i < monCounter; i++) {
            BaseMonster* m = monsters[i];
            if (!isAlive(m) || !isupper(m->getName())) continue;
...
void space::move(pos &start, BaseMonster* mon){
    ...
    if (isAlive(mon)){ // когда монстр завершит ход, проверяем, жив ли он
        start = PUniMon;
        mon->location = PUniMon;
    }
    else {
...
void space::findPos(){
    for (int i = 0; i < n; i++){
        for (int j = 0; j < k; j++){
            switch (Data[i][j]) {
                case 'A':
                    PMonA.y = i; PMonA.x = j; break;
                case 'a':
                    PMona.y = i; PMona.x = j; break;
                case 'C':
                    PMonC.y = i; PMonC.x = j; break;
                case 'c':
                    PMonc.y = i; PMonc.x = j; break;
                case 'B':
                    PMonB.y = i; PMonB.x = j; break;
                case 'b':
                    PMonb.y = i; PMonb.x = j; break;
            }
        }
